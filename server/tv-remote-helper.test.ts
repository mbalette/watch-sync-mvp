...
describe("TV remote helper endpoints", () => {
  it("reports implemented helper targets and full target roadmap", async () => {
    ...
    expect(
      targets.targets.map((target: { id: string }) => target.id),
    ).toContain("apple-tv-manual-only");
  });
  ...
  it("routes LG and Samsung helper endpoints through injectable adapters", async () => {
    ...
    const baseUrl = await startHelper({
      ...
      startVizioPairing: async () => {
        calls.push("vizio-pair-start");
        return { pairingToken: "pair-token" };
      },
      confirmVizioPairing: async () => {
        calls.push("vizio-pair-confirm");
        return { authToken: "vizio-token" };
      },
    });

    await expect(
      postJson(`${baseUrl}/lg/pair/start`, { host: "127.0.0.1" }),
    ).resolves.toMatchObject({ ok: true, clientKey: "lg-key" });
    await expect(
      postJson(`${baseUrl}/lg/keypress`, {
        host: "127.0.0.1",
        clientKey: "lg-key",
        command: "play",
      }),
    ...
    await expect(
      postJson(`${baseUrl}/samsung/pair/start`, { host: "127.0.0.1" }),
    ).resolves.toMatchObject({ ok: true, token: "samsung-token" });
    await expect(
      postJson(`${baseUrl}/samsung/keypress`, {
        host: "127.0.0.1",
        key: "KEY_PLAY",
      }),
    ).resolves.toMatchObject({ ok: true });
    ...
    await expect(
      postJson(`${baseUrl}/philips/key`, {
        host: "127.0.0.1",
        key: "PlayPause",
      }),
    ).resolves.toMatchObject({ ok: true });
    await expect(
      postJson(`${baseUrl}/vizio/pair/start`, { host: "127.0.0.1" }),
    ).resolves.toMatchObject({ ok: true, pairingToken: "pair-token" });
    await expect(
      postJson(`${baseUrl}/vizio/pair/confirm`, {
        host: "127.0.0.1",
        code: "1234",
        pairingToken: "pair-token",
      }),
    ).resolves.toMatchObject({ ok: true, authToken: "vizio-token" });
    await expect(
      postJson(`${baseUrl}/vizio/keypress`, { host: "127.0.0.1", key: "play" }),
    ).resolves.toMatchObject({ ok: true });
  ...
  it("rejects browser requests from unapproved origins before routing commands", async () => {
    const calls: string[] = [];
    const baseUrl = await startHelper({
      sendRokuKeypress: async () => {
        calls.push("roku");
      },
    });

    const allowed = await postJson(
      `${baseUrl}/roku/keypress`,
      { host: "192.168.1.2", key: "Play" },
      { Origin: "https://app.kyrosdirect.tech" },
    );
    ...
    expect(blocked).toMatchObject({
      ok: false,
      error: expect.stringMatching(/origin/i),
    });
  ...
  it("routes ADB connect and discrete media keys through injectable runner without shell strings", async () => {
    ...
    await expect(
      postJson(`${baseUrl}/adb/connect`, { host: "192.168.1.50:5555" }),
    ).resolves.toMatchObject({ ok: true, platform: "adb-helper-advanced" });
    await expect(
      postJson(`${baseUrl}/adb/media-key`, {
        host: "192.168.1.50:5555",
        key: "KEYCODE_MEDIA_PLAY",
      }),
    ).resolves.toMatchObject({
      ok: true,
      platform: "adb-helper-advanced",
      key: "KEYCODE_MEDIA_PLAY",
    });
    await expect(
      postJson(`${baseUrl}/adb/media-key`, {
  ...
  it("rejects ADB toggle key without echoing shell-shaped host input", async () => {
    ...
    const toggle = await postJson(`${baseUrl}/adb/media-key`, {
      host: "192.168.1.50:5555",
      key: "KEYCODE_MEDIA_PLAY_PAUSE",
    });
    const injection = await postJson(`${baseUrl}/adb/connect`, {
  ...
  it("posts Home Assistant webhook test and play payloads once through the local helper", async () => {
    ...
    await expect(
      postJson(`${baseUrl}/home-assistant/webhook`, {
    ...
    expect(webhookCalls[0].body).toMatchObject({
      type: "watch_sync_test",
      room_id: "ROOM1",
      countdown_id: "countdown-1",
      issued_at: "2026-05-05T12:00:00.000Z",
    });
    expect(webhookCalls[1].body).toMatchObject({
      type: "watch_sync_go",
      room_id: "ROOM1",
      countdown_id: "countdown-2",
      issued_at: "2026-05-05T12:00:01.000Z",
    });
  ...
  it("rejects non-http Home Assistant webhook URLs without echoing secret URLs", async () => {
    ...
    const response = await postJson(`${baseUrl}/home-assistant/webhook`, {
  ...
  it("returns a safe Home Assistant webhook failure without echoing the secret URL", async () => {
    ...
    expect(response).toMatchObject({ ok: false });
    expect(String(response.error)).toMatch(
      /Home Assistant webhook returned 500/,
    );
    expect(JSON.stringify(response)).not.toContain(secretUrl);
...
async function startHelper(
  deps?: Parameters<typeof createTvRemoteHelperServer>[0],
): Promise<string> {
  ...
  if (!address || typeof address === "string")
    throw new Error("helper server did not expose a TCP port");
...
function readRequestJson(
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("end", () => {
      try {
        resolve(
          body.trim() ? (JSON.parse(body) as Record<string, unknown>) : {},
        );
      } catch (error) {
        reject(error);
      }
