...
export function normalizeRemoteStartRuntimeConfig(input: unknown): RemoteStartRuntimeConfig {
  const raw = typeof input === 'object' && input ? input as Partial<Record<keyof RemoteStartRuntimeConfig, unknown>> : {}
  const audience = raw.remoteStartRuntimeBetaAudience === 'all' || raw.remoteStartRuntimeBetaAudience === 'off' || raw.remoteStartRuntimeBetaAudience === 'internal'
...
export function isRemoteStartInternalBetaUnlocked(urlSearch = typeof window === 'undefined' ? '' : window.location.search, storage: Storage | null = typeof localStorage === 'undefined' ? null : localStorage): boolean {
...
export function isRuntimeBetaFlagEnabled(platform: LinkedTvPlatform, config: RemoteStartRuntimeConfig): boolean {
  switch (platform) {
    case 'roku':
...
export function isRemoteStartPlatformEnabled(platform: LinkedTvPlatform, config: RemoteStartRuntimeConfig, internalUnlocked = false): boolean {
  if (config.remoteStartKillSwitchEnabled) return false
  if (config.remoteStartPublicEnabled) return true
  if (config.remoteStartRuntimeBetaAudience === 'all') return isRuntimeBetaFlagEnabled(platform, config)
  if (config.remoteStartRuntimeBetaAudience === 'internal' && internalUnlocked) return isRuntimeBetaFlagEnabled(platform, config)
  return false
}
